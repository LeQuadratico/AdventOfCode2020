...
namespace Day_07_1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int amountGoldBags = 0;
            foreach (Bag b in allBags)
            {
                bool includesGold = false;
                foreach (KeyValuePair<Bag, short> pair in b.StartGetIncludedBagsRecursively())
                {
                    if (pair.Key.name == "shiny gold")
                        includesGold = true;
                }
                if (includesGold)
                {
            ...
            Console.WriteLine("\nThere are " + amountGoldBags + " bags able to hold a golden bag.");
        }
