...
namespace Day_19_1
{
    class Program
    {
        ...
        {
            Console.WriteLine("AdventOfCode - Day 19-1\n");
            ...
            {
                if (ruleZero.MatchesRule(lines[i]).success)
                {
                    sum++;
                }
            }
                ...
                foreach (List<short> list in possibleRulesShort)
                {
                    ...
                    if (result1.success)
                    {
                        if (list.Count < 2)
                        {
                            ...
                            else if (result1.length == input.Length)
                                return new CheckResult(true, result1.length);
                            else
                                return new CheckResult(false, result1.length);
                        }
                        ...
                        if (result2.success)
                        {
                            ...
                            else if ((result1.length + result2.length) == input.Length)
                                return new CheckResult(true, (short)(result1.length + result2.length));
                            else
                                return new CheckResult(false, (short)(result1.length + result2.length));
                        }
                    }
                }
                return new CheckResult(false, 1);
